#!/usr/bin/env python3
"""Collect and classify exp05 client container exits from Docker."""
...
def classify_failure_phase(status: str, exit_code: int | None, logs_tail: str, latest_round_seen: int | None) -> str:
    text = logs_tail.lower()
    saw_round = latest_round_seen is not None
    saw_fit_done = "fit done:" in text
    saw_eval = "evaluate:" in text
    saw_dataset = (
        "files already downloaded and verified" in text
        or "train idx" in text
        or "eval idx" in text
    )
    ...
    if not saw_round:
        if saw_dataset:
            return "failed_on_startup_or_loading"
        return "failed_on_startup_or_connection"
    if saw_round and not saw_fit_done:
        return "failed_during_local_training"
    if saw_fit_done or saw_eval:
        return "failed_after_partial_progress"
    return "failure_phase_unknown"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect exp05 client failures from Docker state/logs")
    parser.add_argument("--compose-file", default="docker/docker-compose.exp05_validation.yml")
    parser.add_argument("--outputs-dir", default="outputs/exp05_extreme_memory_limits")
    parser.add_argument("--run-id", default="", help="Optional run_id (e.g., run_20260310_120000)")
    ...
    for service_name, service_cfg in services.items():
        ...
        if inspect_proc.returncode != 0:
            ...
            failure_phase = "failed_on_startup_or_connection" if log_exit else "not_found_or_not_created"
            item = {
        ...
        failure_phase = classify_failure_phase(status, exit_code, logs_tail, latest_round_seen)
        ...
        item = {
            ...
            "latest_round_seen": latest_round_seen,
            "likely_cause": likely_cause,
            "failure_phase": failure_phase,
        }
        report["clients"].append(item)
