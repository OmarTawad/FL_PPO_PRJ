#!/usr/bin/env python3
"""Collect and classify exp08 client container exits from Docker."""
...
PRUNING_RE = re.compile(
    r"pruning\s+round=(\d+).*?requested=(\d+).*?applied=(\d+).*?"
    r"amount_requested=([0-9.]+).*?amount_applied=([0-9.]+)",
    re.IGNORECASE,
)
...
def parse_last_pruning(logs_tail: str) -> tuple[bool | None, float | None]:
    matches = list(PRUNING_RE.finditer(logs_tail))
    if not matches:
        return None, None

    last = matches[-1]
    requested = int(last.group(2))
    applied = int(last.group(3))
    amount_requested = float(last.group(4))
    amount_applied = float(last.group(5))

    pruning_active = bool(applied == 1)
    amount = amount_applied if pruning_active else amount_requested
    if requested == 0:
        return False, 0.0
    return pruning_active, amount
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect exp08 client failures from Docker state/logs")
    parser.add_argument("--compose-file", default="docker/docker-compose.exp08_validation.yml")
    parser.add_argument("--outputs-dir", default="outputs/exp08_pruning_weak10")
    ...
    for service_name, service_cfg in services.items():
        ...
        if inspect_proc.returncode != 0:
            ...
            item = {
                ...
                "pruning_active_last_seen": None,
                "pruning_amount_last_seen": None,
            }
        ...
        pruning_active_last_seen, pruning_amount_last_seen = parse_last_pruning(logs_tail)
        ...
        item = {
            ...
            "likely_cause": likely_cause,
            "failure_phase": failure_phase,
            "pruning_active_last_seen": pruning_active_last_seen,
            "pruning_amount_last_seen": pruning_amount_last_seen,
        }
        report["clients"].append(item)
        cause_counts[likely_cause] = cause_counts.get(likely_cause, 0) + 1
        phase_counts[failure_phase] = phase_counts.get(failure_phase, 0) + 1
